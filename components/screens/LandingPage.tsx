  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      if (!validationResult.valid) {
        alert(validationResult.error || '파일 검증에 실패했습니다.');
        setLoading(false);
        return;
      }
      ...
      // 3. Gemini API 호출
      setLoading(true, 'AI가 평면도를 분석 중...', 50);
      ...
      <header className="sticky top-0 z-50 w-full border-b border-stone-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleUploadClick} className="bg-primary-brown hover:bg-stone-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-all shadow-lg shadow-stone-200">
              시작하기
            </button>
            <input
              ref={fileInputRef}
